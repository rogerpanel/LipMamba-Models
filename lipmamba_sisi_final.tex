%% ============================================================
%% LipMamba — Системы и средства информатики (СиСИ) submission
%% Publisher: ФИЦ ИУ РАН, ssi@frccsc.ru
%% Target VAK specialty: 2.3.1 (К2)
%%
%% Paper: Lipschitz-Constrained Selective State-Space Models
%%        with PAC-Bayesian Certificates for Certified Robustness
%%        Against Hidden State Poisoning in Language Models
%%
%% Compilation: pdflatex + biblatex/native bibitems. Two passes.
%% Layout: single-column A4, Times New Roman 12pt, RU+EN bilingual
%% header per СиСИ author guidelines (see ipiran.ru/journal/collected/).
%% ============================================================
...
%% ============================================================
%% RUSSIAN-LANGUAGE HEADER (per СиСИ author guidelines)
%% ============================================================

\begin{otherlanguage}{russian}
\noindent\textbf{УДК 004.056.5 + 004.85}
\hfill
...
\begin{center}
{\large\bfseries
LipMamba: Селективные модели пространства состояний с
ограничением Липшица и сертификатами PAC-Байеса для
гарантированной устойчивости к отравлению скрытого состояния
...
\begin{center}
{\large\bfseries
\modelname{}: Lipschitz-Constrained Selective State-Space Models
with PAC-Bayesian Certificates for Certified Robustness Against
Hidden State Poisoning in Language Models
\par}
...
\noindent\textbf{Keywords:} selective state-space models, Mamba, Lipschitz continuity, PAC-Bayes, certified robustness, hidden state poisoning, language models, intrusion detection systems.
...
\noindent\textbf{For citation:} Anaedevha~R.\,N., Trofimov~A.\,G., Borodachev~Y.\,V. \modelname{}: Lipschitz-Constrained Selective State-Space Models with PAC-Bayesian Certificates for Certified Robustness Against Hidden State Poisoning in Language Models // \ru{Системы и средства информатики}. \ru{Год}. \ru{Т}. \ldots, \ru{№} \ldots. \ru{С}. \ldots--\ldots\ DOI: \ldots
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{figures/fig1_LipMamba_arch.pdf}
...
\begin{table}[!t]
\centering
\caption{Main results on RoBench-25 (LLM-side) and four IDS datasets
averaged. PACC at HiSPA $\ell\!=\!24$. ASR on HarmBench scored by
HarmBench-CLS. \emph{Cert.\ Acc.} is certified accuracy at the
listed $\epsilon^*$; for empirical defenses, certificate is N/A.
Best in \textbf{bold}; second-best \underline{underlined}.}
\label{tab:main}
\renewcommand{\arraystretch}{1.1}
\setlength{\tabcolsep}{2.2pt}
\footnotesize
\newcommand{\rh}[1]{\rotatebox[origin=l]{70}{\parbox{2.1cm}{\raggedright #1}}}
\begin{tabular}{@{}l
  *{5}{c}@{\hskip 4pt}!{\vrule width 0.4pt}@{\hskip 4pt}*{4}{c}@{}}
\toprule
& \multicolumn{5}{c@{\hskip 4pt}}{\textbf{LLM (RoBench-25)}}
& \multicolumn{4}{c}{\textbf{IDS (4-Dataset Avg.)}} \\[1pt]
\cmidrule(lr){2-6}\cmidrule(l){7-10}
\rh{\textbf{Method}}
& \rh{Accuracy}
& \rh{PACC}
& \rh{Attack Success Rate $\downarrow$}
& \rh{Certified Accuracy}
& \rh{Certified Radius $\epsilon^*$}
& \rh{Macro-F1}
& \rh{PACC}
& \rh{Expected Calibration Error $\downarrow$}
& \rh{Latency} \\[2pt]
\midrule
Mamba (unconstr.)
  & 89.7 & 23.4 & 92.1 & 19.2 & 0.04
  & 94.7 & 31.5 & 0.058 & 0.5\,ms \\
Mamba-2
  & 90.3 & 25.8 & 89.4 & 21.8 & 0.05
  & 95.1 & 33.7 & 0.052 & 0.4\,ms \\
Jamba-1.7-Mini
  & 91.2 & 18.6 & 88.7 & 12.4 & 0.03
  & 93.8 & 28.9 & 0.061 & 1.2\,ms \\
MambaShield~\cite{anaedevha2025mambashield}
  & \underline{91.6} & 64.3 & 47.2 & N/A & N/A
  & \underline{95.9} & 71.8 & \underline{0.042} & \textbf{0.5\,ms} \\
CLASP~\cite{lemercier2026clasp}
  & 89.7 & 67.1 & 38.5 & 55.7 & N/A
  & --- & --- & --- & 1.0\,ms \\
SpectralGuard~\cite{lemercier2026spectralguard}
  & 89.4 & 71.4 & 35.6 & 62.3 & 0.10
  & --- & --- & --- & 0.9\,ms \\
\midrule
\modelname{}-130M
  & 90.8 & \underline{82.7} & \underline{32.4} & \underline{77.1} & \underline{0.15}
  & 95.6 & \underline{84.2} & 0.038 & 0.6\,ms \\
\modelname{}-370M
  & \textbf{91.9} & \textbf{85.3} & \textbf{30.9} & \textbf{80.4} & \textbf{0.18}
  & \textbf{96.4} & \textbf{86.7} & \textbf{0.034} & 0.8\,ms \\
\bottomrule
\end{tabular}
\end{table}
...
\node[font=\footnotesize, red!80, anchor=west] at (axis cs:0.182, 80) {$\epsilon^* = 0.18$};
\end{axis}
\end{tikzpicture}
...
\begin{axis}[
    ...
    legend style={
        ...
        legend columns=3, font=\footnotesize,
...
\node[font=\scriptsize, black!60, anchor=south west] at (axis cs:1.01, 0.4) {$1$ ms SLO};
\end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.96]
  ...
  \foreach \level in {20,40,60,80,100} {
    ...
    \node[font=\footnotesize, gray!60] at (90:\r+0.15) {\level};
  }
  \foreach \i in {1,...,\naxes} {
    \pgfmathsetmacro{\angle}{\angleincrement*\i-90}
    \draw[gray!40, line width=0.3pt] (0,0) -- (\angle:2.7);
    \pgfmathsetmacro{\labelr}{3.15}
    \node[font=\footnotesize, align=center, text width=1.4cm] at (\angle:\labelr) {%
  ...
  \node[font=\footnotesize, anchor=east] at (-2.4, -3.9) {\textcolor{red!80}{\rule{8pt}{1.5pt}} \textbf{\modelname{}-370M}};
  \node[font=\footnotesize, anchor=center] at (0, -3.9) {\textcolor{blue!50}{- - -} MambaShield};
  \node[font=\footnotesize, anchor=west] at (1.5, -3.9) {\textcolor{orange!80}{$\cdots$} CLASP};
\end{tikzpicture}
...
\begin{axis}[
    ...
    legend style={
        at={(0.5, 1.05)}, anchor=south,
        legend columns=3, font=\footnotesize\bfseries,
        column sep=8pt,
...
\node[font=\footnotesize\bfseries, red!70!black, anchor=south] at (axis cs:60, 4.4) {HiSPA trigger};
\node[font=\footnotesize\bfseries, red!70!black, anchor=south] at (axis cs:60, 4.05) {$\ell=24$};
...
\node[font=\footnotesize\bfseries, orange!80!black, anchor=west] at (axis cs:108, 2.2) {$\alpha_{\min}\norm{\mathbf{h}_0}$};
...
\begin{axis}[
    ...
    y tick label style={font=\footnotesize\bfseries},
    grid=major, grid style={gray!20},
    legend style={at={(0.5, 1.05)}, anchor=south, font=\footnotesize\bfseries,
...
% Annotate each point
\node[font=\footnotesize\bfseries, anchor=north]      at (axis cs: 89.7, -1)  {Mamba};
\node[font=\footnotesize\bfseries, anchor=north]      at (axis cs: 90.3, -1)  {Mamba-2};
\node[font=\footnotesize\bfseries, anchor=south west] at (axis cs: 91.0, 1)   {Jamba};
\node[font=\footnotesize\bfseries, anchor=south west] at (axis cs: 91.55, 2)  {MShield};
\node[font=\footnotesize\bfseries, anchor=west]       at (axis cs: 89.85, 50.3) {CLASP};
\node[font=\footnotesize\bfseries, anchor=west]       at (axis cs: 89.55, 56.4) {SpectralG.};
\node[font=\footnotesize\bfseries, anchor=south]      at (axis cs: 90.8, 75.5) {\textbf{LipMamba-130M}};
\node[font=\footnotesize\bfseries, anchor=south]      at (axis cs: 91.9, 82.5) {\textbf{LipMamba-370M}};
...
The authors thank the faculty and specialists of the departments Cybernetics and Neural Network laboratory for their immense contributions which aid the completion of this research project.  The authors declare no competing financial interests whatsoever.
